...
d3.json("climate.json").then((data) => {
  ...
  function updateChart(m) {
    const bins = d3
      .bin()
      .thresholds(10)
      .value((d) => d.average)(data[m]); //filter
      ...
      .join(
        ...
        (update) => {
          update
            ...
            .transition()
            .duration(750)
            .attr("y", (d) => y(d.length)) //change the y position
            .attr("height", (d) => height - margin.bottom - y(d.length)); //change the height
  ...
  // event listener
  d3.selectAll("select") //selecting the dropbox
    .on("change", function (event) {
      const m = event.target.value; //Ex january, february, march
      updateChart(m);
    });
});
