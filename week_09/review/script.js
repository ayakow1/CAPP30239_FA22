...
function ring2() {
  ...
  svg
    ...
    .attr("x", 0)
    .attr("y", (d, i) => `${i * 1.1}em`)
    .attr("font-weight", (d, i) => (i ? null : "bold")) //hide label based on size
    .text((d) => d);
...
function ring3() {
  ...
  d3.select("#chart2b").append("div").node().innerHTML = swatchHTML;
}
function legend() {
  d3.select("#chart4")
    .node()
...
function network() {
  d3.json("color-network.json").then((data) => {
    ...
    simulation.on("tick", () => {
      let link = svg.selectAll("line").data(links);
      ...
      let node = svg.selectAll("circle").data(nodes);
